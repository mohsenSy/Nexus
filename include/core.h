...
SC_MODULE(core) {
  ...
  sc_inout<bool> memory_data_v;;
  sc_inout<bool> memory_data_f;
  sc_inout<sc_int<32> > memory_data;
  sc_out<bool> memory_rw;
  ...
  SC_CTOR(core): taskFifo(BUFFER_DEPTH)   {
    ...
    memory_addr_v.initialize(false);
    previous_task.id = 0;
    mem_cycles = 0;
    num_tasks = 0;
